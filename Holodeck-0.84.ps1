...
# 01/23/2025
#   - Bug fixes
#   - Update to include common local ESX host settings (DNS, NTP, SSH, SYSLOG, License)
#
# 07/02/24
...
$labPriDNS = '192.168.10.30' # <-----------------------------------------UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT (ESX HOST)
$labSecDNS = '8.8.8.8' # <-----------------------------------------------UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT (ESX HOST)
$NTPServers = '129.6.15.28' # <------------------------------------------UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT (ESX HOST)
$Syslog = '192.168.10.20' # <--------------------------------------------UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT (ESX HOST)
$ESXiLicenseKey = '' # <-------------------------------------------------UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT (ESX HOST)
...
$HoloDeckPath = 'E:\Holodeck' # <-------------------------------------------------- UPDATE ACCORDINGLY TO YOUR LAB ENVIRONNMENT
...
$DSFolder = '' # Datastore folder / subfolder name if any (i.e. 'iso\' or '' for ESX local datastore root)
...
# Prior Holodeck Console version for reference only
...
#
###############################################################################################################################
# EDIT WITH CAUTIOUS BEYOND THIS POINT - ALL VARIABLES ARE SET TO MATCH DEFAULT HOLODECK DOCUMENTATION
###############################################################################################################################
#
...
Function CreateHoloConsoleISO {
    ...
    if($CreateHoloConsoleISO -gt 0) {
        ...
        foreach( $i in $ExistHCISO) { 
            Rename-Item $i.FullName ($i.BaseName+".isoold")
...
Function PrepHost {
    # Configure local ESX Host Settings
    param(
        [Parameter(Mandatory=$false)]
        [int]$PrepareHost
        )
    if( $PrepareHost -eq 1) {
        # Apply the license to the ESXi host
        My-Logger "Applying ESXi license for ESX Host $VIServer ..."
        Set-VMHost -VMHost $viConnection -LicenseKey $ESXiLicenseKey
        # Opt out of CEIP
        My-Logger "Opt out of VMware CEIP on ESX Host $VIServer ..."
        Get-AdvancedSetting -Entity $viConnection -Name UserVars.HostClientCEIPOptIn | Set-AdvancedSetting -Value 2 -Confirm:$false
        # Configure DNS settings
        My-Logger "Configuring DNS settings on ESX Host $VIServer ..."
        Get-VMHost | Get-VMHostNetwork | Set-VMHostNetwork -DnsAddress $labPriDNS, $labSecDNS 
        # Configure NTP settings
        My-Logger "Configuring NTPsettings on ESX Host $VIServer ..."
        ...
        # Configure Syslog
        My-Logger "Configuring Syslog on ESX Host $VIServer ..."
        # Get-VMHost | Set-VMHostSysLogServer -SysLogServer $SyslogServer
        # Configure SSH
        My-Logger "Starting SSH service on $VIServer ..."
...
Function CreateSiteNetwork {
    ...
    if($CreateSiteNetwork -eq 1) {
        ...
        if( $vSwtichSite1.Name -eq $HoloDeckSite1vSwitch) {
            ...
            My-Logger "Creating Virtual Switch '$HoloDeckSite1vSwitch' on ESX host $VIServer for Site #1"
...
#
$PrepareHost = 0
#   0 - No ESX Host local settings preparation
#   1 - Configure common local settings on ESX host
#
$EnableSiteNetwork = 0
