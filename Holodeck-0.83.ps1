...
#
$NewHoloConsoleISO = 0 # (0 - Use existing HoloConsole Custom ISO, 1 - Create a fresh HoloConsole Custom ISO, 2 - Create a fresh HolocConsole Custom ISO with Temp dir cleanup)
...
Function CreateHoloConsoleISO {
    ...
    if($CreateHoloConsoleISO -gt 0) {
        $ExistHCISOCount = 0
        $ExistHCISO = Get-ChildItem -Path $HoloConsoleISOPath -Filter "*.iso"
...
Function VLC {
       ...
       param(
        ...
        Do {
            My-Logger "Waiting/confirming HoloConsole VM '$HoloConsoleVMName' is online" 1
            $HoloConsoleVM = Get-VM -Name $HoloConsoleVMName
            $HCCondition = $HoloConsoleVM.ExtensionData.Guest.ToolsStatus
            Start-Sleep 5
        } Until (
            $HoloConsoleVM.ExtensionData.Guest.ToolsStatus -eq 'toolsOK') # toolsNotRunning vs toolsOK
            My-Logger "HoloConsole VM '$HoloConsoleVMName' is online"
            Start-Sleep 5
        if($VLC -gt 0) {
            # $credMyGuestCred = Get-Credential vcf\Administrator
            # Copy VLC files (headless.ini, Holo-D#-50.ini) from Build Host to VLC Site 1 folder on HoloConsole VM
            My-Logger "Copy latest VLC configuration files from Build Host to HoloConsole VM"
            Copy-VMGuestFile -VM $HoloConsoleVMName -LocalToGuest -Source $VLCSite1Path\$VLCHoloD1INI -Destination 'C:\VLC\VLC-Holo-Site-1\' -GuestUser 'vcf\administrator' -GuestPassword 'VMware123!'
            ...
            My-Logger "Copy action completed"
        } elseif ($VLC -eq 2) {
            ############### WIP - Still need testing #####################################
            # Holo-Site-1-vcf-ems-public.json
            My-Logger "Initate VLC bring-up in headless mode"
...
Function ShutdownMgmtDomain {
    # Shut down nested VCF Management Domain
    param(
     [Parameter(Mandatory=$true)]
     [int]$ShutdownMgmtDomain
     )
     if($ShutdownMgmtDomain -gt 0) {
        # Shut down all nested management VMs
        foreach ($NestedVM in $NestedMgmtVM){
            My-Logger "Shutting down VM '$NestedVM'"
            Stop-VMGuest -Server $NestedMgmtVC -VM $NestedVM -Confirm:$false
            Do {
                My-Logger "Waiting for VM '$NestedVM' to be offline" 1
                $ShutDownVM = Get-VM -Server $NestedMgmtVC -Name $NestedVM
                $HCCondition = $ShutDownVM.ExtensionData.Guest.ToolsStatus
                Start-Sleep 10
            } Until (
                $ShutDownVM.ExtensionData.Guest.ToolsStatus -eq 'toolsNotRunning')
            Write-Host "VM '$NestedVM' powered off"
        }
        # Shut down all nested ESXi VMs
        foreach ($NestedVM in $NestedMgmtESX){
            My-Logger "Shutting down VM $NestedVM"
            Stop-VMGuest -Server $viConnection -VM $NestedVM -Confirm:$false
            Do {
                My-Logger "Waiting for VM $NestedVM to be offline" 1
                $ShutDownVM = Get-VM -Server $viConnection -Name $NestedVM
                $HCCondition = $ShutDownVM.ExtensionData.Guest.ToolsStatus
                Start-Sleep 5
            } Until (
                $ShutDownVM.ExtensionData.Guest.ToolsStatus -eq 'toolsNotRunning')
...
#
# My-Logger "Connecting to Nested Management vCenter $NestedMgmtVC ..."
# $NestedMgmtVCConnection = Connect-VIServer $MgmtVC -User 'Administrator@vsphere.local' -Password $VIPassword -WarningAction SilentlyContinue
#
# Install-Module -Name VMware.CloudFoundation.PowerManagement -RequiredVersion 1.2.0.1001	-Force
# Install-Module -Name VMware.CloudFoundation.PowerManagement -RequiredVersion 1.4.0.1001	-Force

# $sddcManagerFqdn = "sddc-manager.vcf.sddc.lab"
# $sddcManagerUser = "administrator@vsphere.local"
# $sddcManagerPass = "VMware123!"
# $sddcDomain = "mgmt-domain"
# & "C:\Program Files\WindowsPowerShell\Modules\VMware.CloudFoundation.PowerManagement\1.4.0.1001\SampleScripts\PowerManagement-ManagementDomain.ps1" -server $sddcManagerFqdn -user $sddcManagerUser -pass $sddcManagerPass -shutdown
# & "C:\Program Files\WindowsPowerShell\Modules\VMware.CloudFoundation.PowerManagement\1.4.0.1001\SampleScripts\PowerManagement-WorkloadDomain.ps1" -server $sddcManagerFqdn -user $sddcManagerUser -pass $sddcManagerPass -sddcDomain $sddcDomain -shutdown -shutdownCustomerVm
